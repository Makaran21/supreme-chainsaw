...
<Card class="image-block group relative gap-0 overflow-hidden rounded p-0" data-order={block.order}>
	<div
		class="absolute top-2 right-2 z-10 flex gap-2 opacity-0 transition-opacity group-hover:opacity-100"
	>
		<Button
			variant="secondary"
			size="icon"
		...
		<Button variant="secondary" size="icon" class="h-8 w-8" onclick={toggleResize}>
	...
	<CardContent class="p-0">
		{#if showUpload}
			<div class="flex flex-col items-center justify-center space-y-4 bg-muted/50 p-8">
				<input
					bind:this={fileInput}
					type="file"
					accept="image/*"
		...
		{:else}
			<AspectRatio ratio={aspectRatio}>
				<img src={imageUrl} {alt} class="h-full w-full object-cover" {width} {height} />
			</AspectRatio>
		{/if}
	</CardContent>
		...
		<CardFooter class="flex flex-col gap-3 py-4">
			<div class="w-full space-y-2">
				<Label for="aspect-ratio">
					Aspect Ratio: {aspectRatio.toFixed(2)} ({aspectRatio > 1
						? 'Landscape'
						: aspectRatio < 1
							? 'Portrait'
							: 'Square'})
