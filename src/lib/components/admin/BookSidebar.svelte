<script lang="ts">
	...
	import BookUpdateDialog from './BookUpdateDialog.svelte';
	...
	$: hasChanges = JSON.stringify(menuData.chapters) !== JSON.stringify(initialMenuData.chapters);
...
<BookUpdateDialog
	...
	open={openEdit}
	onClose={(updatedBook) => {
		menuData.bookTitle = updatedBook?.title || menuData.bookTitle;
		openEdit = false;
	}}
/>
<Sidebar.Root>
	<Sidebar.Content class="min-w-88 overflow-hidden">
		<Sidebar.Group>
							...
							<Tooltip.Provider>
								<Tooltip.Root>
									<Tooltip.Trigger class="flex w-full min-w-0 items-center gap-2">
										<span class="truncate text-xl w-60">{menuData.bookTitle} </span>
									</Tooltip.Trigger>
									<Tooltip.Content>
										<p>{menuData.bookTitle}</p>
									</Tooltip.Content>
								</Tooltip.Root>
							</Tooltip.Provider>
						</div>
			...
			<Sidebar.GroupContent class={cn('mt-22 w-full overflow-y-auto ', 'h-[calc(100vh-9.5rem)]')}>
														...
														<Sidebar.MenuSubItem>
															<div class="flex w-full items-center gap-1">
																...
																<Sidebar.MenuSubButton
																	href={`/admin/book/${menuData.id}/${section.id}?mode=markdown`}
	...
	{:else}
		<Sidebar.Footer class="border-t p-2">
			<div class="flex gap-2">
				<Button variant="outline" class="w-full" href="/admin/book">ទៅ Admin dashboard</Button>
			</div>
		</Sidebar.Footer>
	{/if}
</Sidebar.Root>
